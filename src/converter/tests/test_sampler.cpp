...
// Verifies the Poisson-disk invariant: every pair of accepted points is at
// least `spacing` apart, and every rejected point is closer than `spacing` to
// at least one accepted point.
void expectPoissonInvariant(const std::vector<uint8_t> &accepted, const std::vector<uint8_t> &rejected,
    ...
    for (size_t off = 0; off < rejected.size(); off += rowStride) {
...
TEST(PoissonDiskSampler, LinearChainAcceptsEverySecondPoint) {
    ...
    BoundingBoxd nodeBB({0, 0, 0}, {2, 2, 2});

    PoissonDiskSampler sampler;
    std::vector<uint8_t> accepted, rejected;
    sampler.sample(batch, nodeBB, 1.0, attrs, accepted, rejected);
...
TEST(PoissonDiskSampler, RandomPointsSatisfyPoissonInvariant) {
    ...
    PointBatch batch{buf.data(), positions.size(), 12};
    BoundingBoxd nodeBB({0, 0, 0}, {10, 10, 10});

    PoissonDiskSampler sampler;
    std::vector<uint8_t> accepted, rejected;
    sampler.sample(batch, nodeBB, 0.5, attrs, accepted, rejected);
...
TEST(PoissonDiskSampler, EmptyCandidatesProduceEmptyOutputs) {
    Attribute posAttr = makePositionAttribute({0.01, 0.01, 0.01});
    Attributes attrs;
    attrs.pushAttribute(posAttr);

    PointBatch batch{nullptr, 0, 12};
    BoundingBoxd nodeBB({0, 0, 0}, {1, 1, 1});
...
TEST(PoissonDiskSampler, NonPositiveSpacingAcceptsAllPoints) {
    Attribute posAttr = makePositionAttribute({0.01, 0.01, 0.01});
    Attributes attrs;
    attrs.pushAttribute(posAttr);
    ...
    PoissonDiskSampler sampler;
    std::vector<uint8_t> accepted, rejected;
    sampler.sample(batch, nodeBB, 0.0, attrs, accepted, rejected);
